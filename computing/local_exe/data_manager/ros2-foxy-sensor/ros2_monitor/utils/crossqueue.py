...
class CrossQueue:
    def __init__(self, data_key, address_keys, shape, data_type, max_data_size, max_length=5, flag="IPC_CREAT"):
        ...
        for data_key in self.data_queue:
            self._memories.append(create_shared_memory(data_key, max_data_size, flag=flag))
        # Shared Memory keys for time_stamp
        self.time_queue = [data_key + max_length + i for i in range(max_length)]
        for time_key in self.time_queue:
            self._memories.append(create_shared_memory(time_key, 32, flag=flag))
        ...
        for front_key in self.front_keys:
            ...
            self.front_memories[front_key] = create_shared_memory(front_key, 32, flag=flag)
            self._memories.append(self.front_memories[front_key])
            # Check whether is writing flag or not
            front_flag = front_key
            self.front_locks[front_key] = CrossContainerLock(front_flag, flag=flag)
    ...
    def put(self, data, timestamp):
        ...
        if self.isFull():
            ...
            for front_key in self.front_keys:
                ...
                self.fronts[front_key] = (self.fronts[front_key] + 1) % self.max_length
                write_to_memory(self.front_memories[front_key], str(self.fronts[front_key]))
    ...
    def get(self, address_key=None):
        ...
        data_memory = sysv_ipc.SharedMemory(self.data_queue[self.fronts[address_key]])
        time_memory = sysv_ipc.SharedMemory(self.time_queue[self.fronts[address_key]])
        ...
        self.fronts[address_key] = (self.fronts[address_key] + 1) % self.max_length
        write_to_memory(self.front_memories[address_key], str(self.fronts[address_key]))
        self.front_locks[address_key].release()

        return data.reshape(self.shape), timestamp
    ...
    def gettime(self, address_key=None, top=1):
        ...
        self.front_locks[address_key].acquire()
        try:
            self.fronts[address_key] = int(read_from_memory(self.front_memories[address_key]))
        except:
            pass
        self.front_locks[address_key].release()
    ...
    def isFull(self):
        for front_key in self.front_keys:
            try:
                self.fronts[front_key] = int(read_from_memory(self.front_memories[front_key]))
            except:
                pass
            try:
                self.rear = int(read_from_memory(self.rear_memory))
    ...
    def isEmpty(self, address_key=None):
        if address_key:
            ...
            address_keys = self.front_keys[0]
